...
public class ITRequestWorkflow {
    ...
    private void wfLogic(WorkflowThread wf) {
        WfRunVariable email = wf.addVariable("email", VariableType.STR).required().searchable();
        WfRunVariable item = wf.addVariable("item", VariableType.STR).required();
        WfRunVariable justification = wf.addVariable("justification", VariableType.STR).required();
        ...
        UserTaskOutput approvalOutput = wf.assignUserTask("approve-it-request", userId, userGroup)
                .withNotes(wf.format("User {0} is requesting item {1}. Justification: {2}", email, item, justification));
        ...
        wf.doIfElse(wf.condition(isApproved, Comparator.EQUALS, true), ifBody -> {
            ...
            LHFormatString emailBody = elseBody.format("You request for {0} was rejected because {1}!", item, comments);
